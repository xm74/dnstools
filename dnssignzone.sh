...
# v.20260705 (c)2016-2026 by Max Kostikov W: http://kostikov.co E: max@kostikov.co
#
...
# Zone is DNSSEC
if [ $(ls $zonedir/*.key | wc -l) -eq 0 ]; then
        echo "Zone have not any keys!"
        exit 1
fi

# sigopt="-n -p -t 100 -s `head -c 512 /dev/random | shasum | cut -b 1-16`"
sigopt="-n -t 0 -i $(date -u -v-86400S +%Y%m%d%H%M%S) -e $(date -u -v+${arcage}S +%Y%m%d%H%M%S)"
